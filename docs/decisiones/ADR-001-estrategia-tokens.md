...
Para el acceso desde cualquier aplicación de rol se admite un puente SSO limitado, no la transferencia de cookies ni de JWT entre aplicaciones: un usuario autenticado solicita `POST /api/v1/auth/sso/code` indicando la audiencia (`admin`, `residente` o `vigilante`) y recibe un código opaco aleatorio, solo si su rol corresponde a esa audiencia. La migración `V6__create_admin_sso_codes.sql` persiste únicamente su hash SHA-256, usuario, audiencia, vencimiento de 60 segundos y consumo — el nombre de la tabla (`admin_sso_codes`) es histórico, nació solo para administración en GR-30 y no se renombró al generalizarse en GR-151 porque no hay ninguna razón de esquema para hacerlo. La aplicación de destino lo canjea una sola vez en `POST /api/v1/auth/sso/exchange`; ese endpoint vuelve a validar la correspondencia audiencia-rol y usa `AuthTokenService.issueTokens(...)`, por lo que conserva las mismas cookies y políticas de sesión. El canje exige CSRF y se protege contra doble uso mediante bloqueo pesimista en PostgreSQL.

La correspondencia audiencia-rol (`admin`→`ADMINISTRACION`, `residente`→`RESIDENTE`, `vigilante`→`VIGILANTE`) es la única fuente de verdad y se aplica dos veces: al emitir el código (el rol del solicitante debe corresponder a la audiencia pedida) y al canjearlo (el rol del usuario dueño del código, revalidado en ese momento, debe seguir correspondiendo). Una audiencia desconocida responde `400`; una audiencia que no corresponde al rol, `403`.
...
- El puente SSO tampoco crea un emisor paralelo: solo convierte una credencial opaca de corta vida en las cookies habituales. El código no se registra, no se incluye en errores y no es reutilizable.
